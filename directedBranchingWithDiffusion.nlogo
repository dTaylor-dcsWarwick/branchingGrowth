;; basic filamentous growth - blind agents with basic diffusion
;; Cell density does -not- affect diffusion rate
;; Unlike basic no-diffusion growth, biofilm growth covers the entire
...
to setup-patches
  ask patches [ set pcolor tile_max ]
end

to setup-turtles
  create-turtles 1 [ set energy cell_spawn_cost set branch 0 set age 0 ]
end

to go
  ;;regen-patches ;; all tiles regenerate 1 nutrient to max 10
  check-color ;;
  diffuse-patches ;; all tiles
...
to spawn-turtles
  ask turtles [
    if energy >= cell_spawn_cost [
      let p max-one-of neighbors [pcolor]
      ifelse [pcolor] of p > pcolor [
        hatch 1 [face p fd 1 set energy 0]
      ][
        hatch 1 [fd 1 set energy 0]
      ]
      set energy energy - cell_spawn_cost
    ]
  ]
end
